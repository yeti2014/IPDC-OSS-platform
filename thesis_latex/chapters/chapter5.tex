...
The category classifier reaches perfect accuracy on the synthetic test set, reflecting the clearly distinct vocabulary patterns across IPDC's eleven service types. The priority predictor's moderate accuracy (40\%) is expected: priority assignment involves subjective human judgment that textual features alone cannot fully capture. The time estimator achieves a mean absolute error of 2.63 days, offering tenants useful estimates for planning. Figure~\ref{fig:model1-eval} presents the confusion matrix for service type classification and the scatter plot of predicted versus actual processing days.
...
This chapter documented the implementation and evaluation of the IPDC platform prototype --- approximately 105 source files with dual-layer offline storage, dual-mode authentication, a tiered digital service token system, three AI models, and Workbox-powered PWA configuration. All three AI models show strong performance on synthetic evaluation data: the XGBoost service classifier achieves 100\% category accuracy, the predictive maintenance ensemble reaches $R^{2} = 0.9989$, and the LambdaRank park recommender achieves NDCG@3 of 0.9572; these results establish an architectural baseline pending validation on real IPDC operational data. Technical evaluation confirms all nine offline scenarios pass; Lighthouse scores (89/87/96/92) meet targets; full compatibility holds across Chrome and Opera on both desktop and mobile; a usability evaluation with nine real stakeholders on 10 February 2026 yields a mean SUS score of 74.5 (``Good''); and all seven of Hevner's DSR guidelines are satisfied. The next chapter synthesizes these findings into design principles and outlines directions for future work.
