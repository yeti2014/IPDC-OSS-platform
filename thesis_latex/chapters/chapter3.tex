...
DSR was the right choice for this problem because the problem itself demands a solution, not just an analysis. IPDC's paper-based service delivery in a low-connectivity environment is well documented. A behavioural study could describe it more precisely, but that would not produce anything useful to IPDC. DSR is built for exactly this situation: designing and evaluating artifacts that address real operational problems while also generating transferable knowledge \citep{Hevner2004}.

Three things about the problem made DSR unavoidable. First, IPDC needs a working platform --- a theoretical model would not help. Second, no ready-made solution exists for offline-first industrial park governance in an African context, so design work cannot be skipped. Third, the goal is not just a working system but also generalizable knowledge: design principles and an adaptation framework. That combination --- practical artifact plus theoretical contribution --- is exactly what DSR is designed to produce \citep{Gregor2013}.
...
The research design mixes qualitative and technical methods inside the DSR framework. Qualitative work --- stakeholder surveys, document analysis, Chinese platform review --- built the understanding of the problem and shaped what the platform needed to do. Technical work --- iterative development and systematic testing --- built and validated the artifact. DSR does not prescribe a fixed method; it uses whatever is needed to construct and evaluate the artifact rigorously \citep{Hevner2004}.
...
\small
\begin{tabular}{p{3.0cm}p{2.8cm}p{6.0cm}}
\toprule
