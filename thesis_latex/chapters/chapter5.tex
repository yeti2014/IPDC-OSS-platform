...
This chapter covers the implementation of the IPDC offline-first digital one-stop service prototype and the results of its evaluation. The implementation realizes the architecture described in Chapter~\ref{ch:analysis} as a working Progressive Web Application, comprising over 100 TypeScript source files, three trained machine learning models, and a comprehensive offline-first data management layer. Evaluation then puts the prototype through technical testing of offline functionality, performance measurement, cross-platform compatibility checks, and a structured usability assessment.
...
The Vite build configuration uses manual chunk splitting to improve caching behavior. Three vendor chunks are separated: \texttt{vendor} (React and ReactDOM), \texttt{mui} (Material-UI components), and \texttt{firebase} (Firebase SDKs). Keeping these distinct means application code updates do not force browsers to re-download cached vendor libraries --- a particularly worthwhile optimization where connectivity is limited.
...
\textbf{Token Tiers} reward engagement: basic (0--499 tokens, 0\% discount), silver (500--1,499 tokens, 5\% discount), gold (1,500--4,999 tokens, 10\% discount), and platinum (5,000+ tokens, 20\% discount). Higher tiers also unlock increased monthly request limits and priority support access.
...
\textbf{Model 2 --- Predictive Maintenance:} Given asset attributes, this model produces a failure probability, risk level (low/medium/high), estimated days until failure, and a recommended maintenance action. Identifying assets approaching failure thresholds informs maintenance scheduling decisions.
...
The category classifier reaches perfect accuracy on the synthetic test set, which is perhaps less surprising than it first appears: the eleven IPDC service types have clearly distinct vocabulary patterns, so the separation task is inherently easier than in open-domain classification. The priority predictor's moderate accuracy (40\%) is more telling --- priority assignment involves subjective human judgment that textual features alone cannot fully capture, and 40\% should be read as a realistic ceiling rather than a shortcoming. The time estimator achieves a mean absolute error of 2.63 days, offering tenants useful estimates for planning. Figure~\ref{fig:model1-eval} presents the confusion matrix for service type classification and the scatter plot of predicted versus actual processing days.
...
All three models meet or exceed the figures reported for their Chinese reference implementations. Two important caveats apply. First, the IPDC models were trained and evaluated on \emph{synthetic} datasets designed to reflect realistic IPDC operational patterns (see Section~\ref{sec:ai-training}), whereas the Chinese reference systems are production deployments trained on years of real operational data. Near-perfect scores on a self-generated synthetic dataset are expected and should be interpreted as establishing an architectural baseline rather than a claim of superiority over mature deployed systems. Second, the Chinese benchmark figures (Alibaba ET Industrial Brain: 85--92\%; Huawei FusionPlant: 80--88\%; Tencent WeCity NDCG@3: 0.85) represent values cited in the published literature for comparable tasks and are used here as indicative reference points. All source code, training scripts, and model artefacts are available in the project repository \citep{BerhanuIPDC2025} for independent reproduction and re-evaluation once real IPDC operational data become available. The models are designed for online learning, with Model 3 configured to retrain automatically after every 50 new tenant placements or on a monthly schedule.
...
A usability evaluation was conducted on \textbf{10 February 2026} with nine representative stakeholders from two user groups: IPDC management staff and tenant firm representatives. Participants were recruited purposively to ensure coverage of both roles and multiple industrial park contexts. The evaluator shared the live platform URL (\url{https://ipdc-oss-platform.vercel.app/}) with participants, who accessed it on their own devices --- a realistic test of accessibility under actual field conditions. Table~\ref{tab:sus-participants} lists the participant profiles.
...
Each participant completed four structured task scenarios on the prototype before completing the SUS questionnaire:
...
Following the adjective rating scale established by \citet{Bangor2009}, a score of 74.5 falls in the \textbf{``Good''} range, exceeding the widely cited industry average of 68. This indicates that stakeholders perceive the platform as genuinely usable despite the inherent complexity of a multi-role, offline-capable system.

\subsubsection{Qualitative Feedback}

Post-task open-ended feedback was collected from all participants. Positive themes included:
...
These findings align closely with the requirements gathered from the 49-respondent questionnaire (Section~\ref{sec:data-collection}): both data sources consistently highlight Amharic localization and proactive notification as the most impactful near-term improvements.
...
The platform prototype described in this chapter comprises roughly 105 source files --- dual-layer offline storage, dual-mode authentication, a tiered digital service token system, three AI models, and Workbox-powered PWA configuration. On the technical side, all nine offline test scenarios passed, Lighthouse scores (89/87/96/92) met their targets, and full compatibility was confirmed across Chrome and Opera on both desktop and mobile. On the human side, nine real IPDC stakeholders evaluated the platform on 10 February 2026, returning a mean SUS score of 74.5 --- placing it in the ``Good'' usability band. All seven of Hevner's DSR guidelines are satisfied. The AI model results --- 100\% service classification accuracy, $R^{2} = 0.9989$ for maintenance prediction, NDCG@3 of 0.9572 for park recommendation --- are best understood as an architectural baseline on synthetic data rather than production performance claims; that distinction matters and is easy to overlook when the numbers look this strong. What the evaluation does establish conclusively is that the offline-first architecture works as designed under simulated real-world conditions.
